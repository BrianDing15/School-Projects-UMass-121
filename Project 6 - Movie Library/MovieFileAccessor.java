...
public class MovieFileAccessor extends FileAccessor{
  ...
  */
   public void processLine(String line){
      ...
      Movie lineMovie = null;
      if(lineArray.length == 4){
         lineMovie = new Movie (lineArray[0], lineArray[1], Double.parseDouble(lineArray[2]), Double.parseDouble(lineArray[3]));
      }
      else{
         lineMovie = new Movie (lineArray[0], lineArray[1], lineArray[2], Double.parseDouble(lineArray[3]), Double.parseDouble(lineArray[4]));
      }
      movies.addMovie(lineMovie);
   }
  ...
  */   
   public String movieToCSVString(Movie movie){
      return movie.getTitle() + "," + movie.getDirector() + "," + movie.getGenre() + "," + movie.getPlayTime() + "," + movie.getReleaseYear();
   }
   ...
   public String getMoviesAsCSV(MovieList movieList){
     String result = "";
      for(int i = 0; i < movieList.getMovieList().size(); ++i){
         result = result + movieToCSVString(movieList.getMovieList().get(i)) + "\n";
      }
      return result;
   }
